...
int
mca_io_bbview_file_open(ompi_communicator_t *comm, const char *filename,
			int amode, opal_info_t *info, ompi_file_t *fh)
{
	...
	*/
	...
	/* No lock required for file_open even in multi-threaded scenarios,
	   ...
	   is allowed anyway */
	ret = mca_common_ompio_file_open(comm, filename, amode, info,
					 &data->ompio_fh, use_sharedfp);
...
int
mca_io_bbview_file_close(ompi_file_t *fh)
{
	int ret = OMPI_SUCCESS;
	...
	data = (mca_common_bbview_data_t *)fh->f_io_selected_data;
	if (NULL == data) {
		/* structure has already been freed, this is an erroneous call
		 * to file_close */
		return ret;
	}
...
int
mca_io_bbview_file_preallocate(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE diskspace)
{
	...
	ret = data->ompio_fh.f_comm->c_coll->coll_bcast(
	    &tmp, 1, OMPI_OFFSET_DATATYPE, OMPIO_ROOT, data->ompio_fh.f_comm,
	...
	/* ROMIO explanation
	   ...
	   explicitly write to allocate space. Since there could be holes in the
	   file, we need to read up to the current file size, write it back, and
	   then write beyond that depending on how much preallocation is needed.
	*/
	if (OMPIO_ROOT == data->ompio_fh.f_rank) {
		OMPI_MPI_OFFSET_TYPE prev_offset;
		mca_common_ompio_file_get_position(&data->ompio_fh,
						   &prev_offset);
		...
		for (i = 0; i < cycles; i++) {
			...
			ret = mca_common_ompio_file_read(&data->ompio_fh, buf,
							 len, MPI_BYTE, status);
			if (ret != OMPI_SUCCESS) {
				goto exit;
			}
			ret = mca_common_ompio_file_write(
			    &data->ompio_fh, buf, len, MPI_BYTE, status);
			if (ret != OMPI_SUCCESS) {
				goto exit;
			}
			written += len;
		...
		if (diskspace > current_size) {
			...
			for (i = 0; i < cycles; i++) {
				...
				ret = mca_common_ompio_file_write(
				    &data->ompio_fh, buf, len, MPI_BYTE,
				    status);
				if (ret != OMPI_SUCCESS) {
					goto exit;
				}
		...
		mca_common_ompio_set_explicit_offset(&data->ompio_fh,
						     prev_offset);
	}
...
exit:
	...
	if (diskspace > current_size) {
		data->ompio_fh.f_fs->fs_file_set_size(&data->ompio_fh,
						      diskspace);
	}
	OPAL_THREAD_UNLOCK(&fh->f_lock);
...
int
mca_io_bbview_file_set_size(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE size)
{
	int ret = OMPI_SUCCESS;
	...
	if (OMPI_SUCCESS != ret) {
		opal_output(1,
			    ",mca_io_bbview_file_set_size: error in bcast\n");
		OPAL_THREAD_UNLOCK(&fh->f_lock);
		return ret;
	}
	...
	if (OMPI_SUCCESS != ret) {
		opal_output(
		    1, ",mca_io_bbview_file_set_size: error in fs->set_size\n");
		OPAL_THREAD_UNLOCK(&fh->f_lock);
		return ret;
	}

	ret = data->ompio_fh.f_comm->c_coll->coll_barrier(
	    data->ompio_fh.f_comm,
	    data->ompio_fh.f_comm->c_coll->coll_barrier_module);
	if (OMPI_SUCCESS != ret) {
		opal_output(1,
			    ",mca_io_bbview_file_set_size: error in barrier\n");
		OPAL_THREAD_UNLOCK(&fh->f_lock);
		return ret;
	}
	OPAL_THREAD_UNLOCK(&fh->f_lock);
...
int
mca_io_bbview_file_get_size(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE *size)
{
	int ret = OMPI_SUCCESS;
	mca_common_bbview_data_t *data;
...
int
mca_io_bbview_file_get_amode(ompi_file_t *fh, int *amode)
{
	mca_common_bbview_data_t *data;
...
int
mca_io_bbview_file_get_type_extent(ompi_file_t *fh,
...
int
mca_io_bbview_file_set_atomicity(ompi_file_t *fh, int flag)
{
	...
	data->ompio_fh.f_comm->c_coll->coll_bcast(
	    &tmp, 1, MPI_INT, OMPIO_ROOT, data->ompio_fh.f_comm,
	    data->ompio_fh.f_comm->c_coll->coll_bcast_module);
	...
	bool result;
	if (flag) {
		result = data->ompio_fh.f_fbtl->fbtl_check_atomicity(
		    &data->ompio_fh);
		if (result) {
...
int
mca_io_bbview_file_get_atomicity(ompi_file_t *fh, int *flag)
{
	mca_common_bbview_data_t *data;
...
int
mca_io_bbview_file_sync(ompi_file_t *fh)
{
	int ret = OMPI_SUCCESS;
	mca_common_bbview_data_t *data;
...
static void
mca_io_bbview_file_get_eof_offset(ompio_file_t *fh,
				  ...
				  OMPI_MPI_OFFSET_TYPE *out_offset)
{
	/* a file_seek with SEEK_END might require an actual offset that is
	   ...
	   the provided in_offset value, avoiding gaps in the file view and
	   avoiding to break up an etype.
	*/
	...
	if (fh->f_fview.f_view_size > 0) {
		...
		start_offset = (in_offset / fh->f_fview.f_view_extent) *
			       fh->f_fview.f_view_extent;

		index_in_file_view = 0;
		/* determine block id that the offset is located in and
		   the starting offset of that block */
		while (offset <= in_offset &&
		       index_in_file_view < fh->f_fview.f_iov_count) {
			offset = start_offset +
				 (OMPI_MPI_OFFSET_TYPE)(intptr_t)fh->f_fview
				     .f_decoded_iov[index_in_file_view++]
				     .iov_base;
			if (offset <= in_offset) {
				prev_offset = offset;
			}
...
int
mca_io_bbview_file_seek(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE off, int whence)
{
	int ret = OMPI_SUCCESS;
	mca_common_bbview_data_t *data;
	...
	case MPI_SEEK_END:
		ret = data->ompio_fh.f_fs->fs_file_get_size(&data->ompio_fh,
							    &temp_offset2);
		mca_io_bbview_file_get_eof_offset(&data->ompio_fh, temp_offset2,
						  &temp_offset);
		offset += temp_offset;
	...
	ret = mca_common_ompio_set_explicit_offset(
	    &data->ompio_fh, offset / data->ompio_fh.f_fview.f_etype_size);
	OPAL_THREAD_UNLOCK(&fh->f_lock);
...
int
mca_io_bbview_file_get_position(ompi_file_t *fd, OMPI_MPI_OFFSET_TYPE *offset)
{
	int ret = OMPI_SUCCESS;
	mca_common_bbview_data_t *data = NULL;
	ompio_file_t *fh = NULL;

	data = (mca_common_bbview_data_t *)fd->f_io_selected_data;
	fh = &data->ompio_fh;
...
int
mca_io_bbview_file_get_byte_offset(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset,
				   OMPI_MPI_OFFSET_TYPE *disp)
{
	...
	temp_offset = (long)data->ompio_fh.f_fview.f_view_extent *
		      (offset * data->ompio_fh.f_fview.f_etype_size /
		       data->ompio_fh.f_fview.f_view_size);
	if (0 > temp_offset) {
		OPAL_THREAD_UNLOCK(&fh->f_lock);
		return MPI_ERR_ARG;
	}

	i = (offset * data->ompio_fh.f_fview.f_etype_size) %
	    data->ompio_fh.f_fview.f_view_size;
	index = 0;
	k = 0;
	...
	*disp = data->ompio_fh.f_fview.f_disp + temp_offset +
		(OMPI_MPI_OFFSET_TYPE)(intptr_t)data->ompio_fh.f_fview
		    .f_decoded_iov[index]
		    .iov_base +
		k;
	OPAL_THREAD_UNLOCK(&fh->f_lock);
...
int
mca_io_bbview_file_seek_shared(ompi_file_t *fp, OMPI_MPI_OFFSET_TYPE offset,
			       int whence)
{
	int ret = OMPI_SUCCESS;
	mca_common_bbview_data_t *data;
...
int
mca_io_bbview_file_get_position_shared(ompi_file_t *fp,
