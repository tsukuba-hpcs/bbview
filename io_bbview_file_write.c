			    ...
			    ompi_status_public_t *status)
{
	...
	ret = mca_common_ompio_file_write_at(&data->ompio_fh, offset, buf,
			  ...
			  ompi_request_t **request)
{
	...
	data = (mca_common_bbview_data_t *)fp->f_io_selected_data;
	OPAL_THREAD_LOCK(&fp->f_lock);
	ret = mca_common_ompio_file_iwrite(&data->ompio_fh, buf, count,
					   datatype, request);
	OPAL_THREAD_UNLOCK(&fp->f_lock);
			     ...
			     ompi_request_t **request)
{
	...
	data = (mca_common_bbview_data_t *)fh->f_io_selected_data;
	OPAL_THREAD_LOCK(&fh->f_lock);
	ret = mca_common_ompio_file_iwrite_at(&data->ompio_fh, offset, buf,
			     ...
			     ompi_status_public_t *status)
{
	...
	OPAL_THREAD_LOCK(&fh->f_lock);
	ret = mca_common_ompio_file_write_all(&data->ompio_fh, buf, count,
					      datatype, status);
	OPAL_THREAD_UNLOCK(&fh->f_lock);
				...
				ompi_status_public_t *status)
{
	...
	data = (mca_common_bbview_data_t *)fh->f_io_selected_data;
	OPAL_THREAD_LOCK(&fh->f_lock);
	ret = mca_common_ompio_file_write_at_all(&data->ompio_fh, offset, buf,
						 count, datatype, status);
	OPAL_THREAD_UNLOCK(&fh->f_lock);
			      ...
			      ompi_request_t **request)
{
	...
	OPAL_THREAD_LOCK(&fh->f_lock);
	ret = mca_common_ompio_file_iwrite_all(&data->ompio_fh, buf, count,
					       datatype, request);
	OPAL_THREAD_UNLOCK(&fh->f_lock);
				 ...
				 ompi_request_t **request)
{
	...
	data = (mca_common_bbview_data_t *)fh->f_io_selected_data;
	OPAL_THREAD_LOCK(&fh->f_lock);
	ret = mca_common_ompio_file_iwrite_at_all(&data->ompio_fh, offset, buf,
						  count, datatype, request);
	OPAL_THREAD_UNLOCK(&fh->f_lock);
	return ret;
}
				...
				ompi_status_public_t *status)
{
	...
	OPAL_THREAD_LOCK(&fp->f_lock);
	ret = shared_fp_base_module->sharedfp_write(fh, buf, count, datatype,
						    status);
	OPAL_THREAD_UNLOCK(&fp->f_lock);
				 ...
				 ompi_request_t **request)
{
	...
	OPAL_THREAD_LOCK(&fp->f_lock);
	ret = shared_fp_base_module->sharedfp_iwrite(fh, buf, count, datatype,
						     request);
	OPAL_THREAD_UNLOCK(&fp->f_lock);
				 ...
				 ompi_status_public_t *status)
{
	...
	OPAL_THREAD_LOCK(&fp->f_lock);
	ret = shared_fp_base_module->sharedfp_write_ordered(fh, buf, count,
							    datatype, status);
	OPAL_THREAD_UNLOCK(&fp->f_lock);
				   ...
				   struct ompi_datatype_t *datatype)
{
	...
	ret = mca_io_bbview_file_iwrite_all(fh, buf, count, datatype,
					    &fp->f_split_coll_req);
