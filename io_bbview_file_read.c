...
**   The
**   mca_io_bbview_file_read/write[_at]
**
...
int
mca_io_bbview_file_read(ompi_file_t *fp, void *buf, size_t count,
...
int
mca_io_bbview_file_read_at(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset,
...
int
mca_io_bbview_file_iread(ompi_file_t *fh, void *buf, size_t count,
			 struct ompi_datatype_t *datatype,
...
int
mca_io_bbview_file_iread_at(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset,
			    void *buf, size_t count,
			    struct ompi_datatype_t *datatype,
			    ompi_request_t **request)
{
...
/******************************************************/
int
mca_io_bbview_file_read_all(ompi_file_t *fh, void *buf, size_t count,
			    struct ompi_datatype_t *datatype,
			    ompi_status_public_t *status)
{
	int ret = OMPI_SUCCESS;
...
int
mca_io_bbview_file_iread_all(ompi_file_t *fh, void *buf, size_t count,
			     struct ompi_datatype_t *datatype,
			     ompi_request_t **request)
{
	int ret = OMPI_SUCCESS;
...
int
mca_io_bbview_file_read_at_all(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset,
			       void *buf, size_t count,
			       struct ompi_datatype_t *datatype,
			       ompi_status_public_t *status)
{
...
int
mca_io_bbview_file_iread_at_all(ompi_file_t *fh, OMPI_MPI_OFFSET_TYPE offset,
				void *buf, size_t count,
				struct ompi_datatype_t *datatype,
				ompi_request_t **request)
{
...
/******************************************************/
int
mca_io_bbview_file_read_shared(ompi_file_t *fp, void *buf, size_t count,
			       struct ompi_datatype_t *datatype,
			       ompi_status_public_t *status)
{
	int ret = OMPI_SUCCESS;
	...
	if (NULL == shared_fp_base_module) {
		opal_output(0, "No shared file pointer component found for the "
			       "given communicator. Can not execute\n");
		return OMPI_ERROR;
	}
	OPAL_THREAD_LOCK(&fp->f_lock);
	ret = shared_fp_base_module->sharedfp_read(fh, buf, count, datatype,
						   status);
	OPAL_THREAD_UNLOCK(&fp->f_lock);
...
int
mca_io_bbview_file_iread_shared(ompi_file_t *fh, void *buf, size_t count,
				struct ompi_datatype_t *datatype,
				ompi_request_t **request)
{
	int ret = OMPI_SUCCESS;
	...
	/*get the shared fp module associated with this file*/
	shared_fp_base_module =
	    (mca_sharedfp_base_module_t *)(ompio_fh->f_sharedfp);
	if (NULL == shared_fp_base_module) {
		opal_output(0, "No shared file pointer component found for the "
			       "given communicator. Can not execute\n");
		return OMPI_ERROR;
	...
	OPAL_THREAD_LOCK(&fh->f_lock);
	ret = shared_fp_base_module->sharedfp_iread(ompio_fh, buf, count,
						    datatype, request);
	OPAL_THREAD_UNLOCK(&fh->f_lock);
...
int
mca_io_bbview_file_read_ordered(ompi_file_t *fh, void *buf, size_t count,
				struct ompi_datatype_t *datatype,
				ompi_status_public_t *status)
{
	int ret = OMPI_SUCCESS;
	...
	OPAL_THREAD_LOCK(&fh->f_lock);
	ret = shared_fp_base_module->sharedfp_read_ordered(ompio_fh, buf, count,
							   datatype, status);
	OPAL_THREAD_UNLOCK(&fh->f_lock);
	return ret;
}
...
int
mca_io_bbview_file_read_ordered_begin(ompi_file_t *fh, void *buf, size_t count,
				      struct ompi_datatype_t *datatype)
{
	int ret = OMPI_SUCCESS;
	mca_common_bbview_data_t *data;
	...
	OPAL_THREAD_LOCK(&fh->f_lock);
	ret = shared_fp_base_module->sharedfp_read_ordered_begin(
	    ompio_fh, buf, count, datatype);
	OPAL_THREAD_UNLOCK(&fh->f_lock);
...
int
mca_io_bbview_file_read_ordered_end(ompi_file_t *fh, void *buf,
				    ompi_status_public_t *status)
{
	int ret = OMPI_SUCCESS;
	mca_common_bbview_data_t *data;
	...
	OPAL_THREAD_LOCK(&fh->f_lock);
	ret = shared_fp_base_module->sharedfp_read_ordered_end(ompio_fh, buf,
							       status);
	OPAL_THREAD_UNLOCK(&fh->f_lock);
...
/**********************************************************************/
int
mca_io_bbview_file_read_all_begin(ompi_file_t *fh, void *buf, size_t count,
				  struct ompi_datatype_t *datatype)
{
	int ret = OMPI_SUCCESS;
	ompio_file_t *fp;
	...
	ret = mca_io_bbview_file_iread_all(fh, buf, count, datatype,
					   &fp->f_split_coll_req);
...
int
mca_io_bbview_file_read_all_end(ompi_file_t *fh, void *buf,
				ompi_status_public_t *status)
{
	int ret = OMPI_SUCCESS;
	ompio_file_t *fp;
...
int
mca_io_bbview_file_read_at_all_begin(ompi_file_t *fh,
				     OMPI_MPI_OFFSET_TYPE offset, void *buf,
				     size_t count,
				     struct ompi_datatype_t *datatype)
{
	int ret = OMPI_SUCCESS;
	...
	OPAL_THREAD_LOCK(&fh->f_lock);
	ret = mca_common_ompio_file_iread_at_all(
	    fp, offset, buf, count, datatype, &fp->f_split_coll_req);
	OPAL_THREAD_UNLOCK(&fh->f_lock);
	fp->f_split_coll_in_use = true;
	return ret;
}
...
int
mca_io_bbview_file_read_at_all_end(ompi_file_t *fh, void *buf,
				   ompi_status_public_t *status)
{
	int ret = OMPI_SUCCESS;
	mca_common_bbview_data_t *data;
